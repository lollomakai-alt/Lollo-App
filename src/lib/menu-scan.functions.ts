...
const PROMPT =
  "Sei un assistente esperto nell'estrazione di dati da menu di ristoranti, volantini o grafiche in stile social media (IG Stories). " +
  "Analizza l'immagine indipendentemente dai font utilizzati, dai colori di sfondo o dai box grafici. " +
  "Estrai TUTTE le voci con un proprio nome e un proprio prezzo (piatti, panini, sushi, fritti, bevande, ecc.). " +
  "Restituisci ESCLUSIVAMENTE un array JSON valido, senza testo aggiuntivo e senza blocchi markdown. " +
  'Ogni oggetto deve avere: ' +
  '"name" (string, il nome esatto del piatto così come scritto, es. "Piña Rice", "Tornado", "Big Kahuna"), ' +
  '"description" (string, l\'elenco degli ingredienti così come scritti sotto il nome del piatto, es. "Pomodoro, mozzarella, basilico"; stringa vuota se assente), ' +
  ...
  '"category" (string, il titolo della sezione/box del menu sotto cui si trova il piatto in questa immagine, es. "Pizze", "Sushi", "Panini", "Dolci", "Antipasti"; se il piatto non è sotto nessun titolo di sezione visibile, usa il nome che meglio descrive il tipo di piatto).';
    ...
    return list
      ...
      .map((d) => {
        ...
        return {
          ...
          category: formatText(String(d.category ?? "").trim()) || "Altro",
        } as ScannedDish;
      })
      .filter((dish) => {
