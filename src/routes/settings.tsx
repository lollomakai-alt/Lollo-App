...
function SettingsScreen() {
  const [savedLayout, setSavedLayout] = useState<PosTableLayout[] | null>(null);
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (data) {
          data.forEach((row: Record<string, any>) => {
            ...
            if (row.key === "map_layout" && row.value) {
              try {
                const parsed = typeof row.value === "string" ? JSON.parse(row.value) : row.value;
                if (Array.isArray(parsed)) setSavedLayout(parsed);
              } catch (e) {
                console.error("Errore nel parsing del layout salvato:", e);
              }
            }
          });
        }
    ...
    const localLayout = localStorage.getItem("app_saved_layout");
    if (localLayout) {
      try {
        const parsed = JSON.parse(localLayout);
        if (Array.isArray(parsed)) setSavedLayout(parsed);
      } catch (e) {
        console.error("Errore lettura layout locale:", e);
      }
    }
  }, []);

  // Salvataggio del layout direttamente su Supabase[span_2](start_span)[span_2](end_span)
  const handleSaveLayout = async () => {
    try {
      const currentLayoutData = localStorage.getItem("app_saved_layout") || JSON.stringify(savedLayout || []);
      
      const { error } = await supabase
        .from("settings")
        .upsert({ key: "map_layout", value: currentLayoutData }, { onConflict: "key" });
        
      if (error) throw error;

      if (currentLayoutData) {
        try {
          setSavedLayout(JSON.parse(currentLayoutData));
        } catch (e) {
          // ignora
        }
      }
      ...
      console.error("Errore durante il salvataggio del layout su Supabase:", err);
    }
  };
    ...
    <div className="flex min-h-screen w-full flex-col bg-[#030712] text-slate-100">
      ...
      <main className="mx-auto w-full max-w-4xl flex-1 space-y-6 p-4 lg:p-6">
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-950/80 p-5 shadow-xl space-y-4">
          ...
          <p className="text-xs text-slate-400">
            Salva la disposizione attuale dei tavoli come configurazione di default del locale su Supabase.
          </p>
              ...
              {savedLayout ? (
                <div className="space-y-0.5">
                  <p className="text-sm font-bold text-white">
                    Layout salvato -- {savedLayout.length} tavol{savedLayout.length === 1 ? "o" : "i"}
                  </p>
                  <p className="text-xs text-slate-400">
                    Tavoli: {savedLayout.map((t) => t.label).join(", ")}
                  </p>
                </div>
              ) : (
                <div className="space-y-0.5">
                  <p className="text-sm font-bold text-white">Nessun layout definitivo salvato</p>
                  <p className="text-xs text-slate-400">Disponi i tavoli nella Mappa Live e poi salvali qui come default.</p>
                </div>
              )}
            </div>
            <button
              ...
              className={[
                "inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-xs font-bold uppercase tracking-wider transition-all cursor-pointer shadow-lg",
                layoutSaved ? "bg-emerald-500 text-slate-950 font-black" : "neon-btn-cyan",
              ].join(" ")}
            >
              {layoutSaved ? (
                <>
